...
try:
    ...
    # Fitur ini nanti akan dipakai karena suatu saat gw pakai
    # tapi sementara gw tetap load dulu ke json kalo masih belum siap digunakan
    print("pymongo tidak terinstall, mempersiapkan JSON")
    pass
...
class RoleManager:
    ...
    # metode init yang akan menghubungkan dengan referensi bot
    ...
    async def check_roles(self):
        while self.running:
            # Cek susunan role pada rolelist tiap server
    ...
    async def check_member_role(self, r):
        if r.guild == None or r.member == None:
            # Kalo nggk bisa diterapkan return aja
            return
        if not r.guild.me.guild_permissions.manage_roles:
            # Kalo bot Acinonyx Missing permissions untuk mengatur role, return aja
            return
        # kalo bot Acinonyx memiliki permissions lanjut kebawah
        if len(r.add_roles):
            try:
        ...
        if len(r.add_roles) and len(r.rem_roles):
            # Pause dulu untuk beberapa saat sebelum melanjutkan prosesnya jika dibutuhkan
    ...
    def _update(self, member, *, add_roles = [], rem_roles = []):
        # Menambahkan update role ada didalam database
        # atau tambahkan yang baru jika tidak ada dalam database
        if not type(member) == discord.Member:
            # Tidak dapat merubah role tanpa guild
            return
        # Cek dulu apakah ada add_roles diatasnya sesuai posisi
        ...
        for a in add_roles:
            ...
            if a.position < top_index:
                # Kalo didalam new_add ada yang baru tambahkan saja
                new_add.append(a)
        for r in rem_roles:
            if not r:
                continue
            if r.position < top_index:
                # list dalam new_rem bisa dibuang
                new_rem.append(r)
        if len(new_add) == 0 and len(new_rem) == 0:
            # Kalo tidak ada perubahan tetapkan nilainya sama
            return
        self.q.put_nowait(MemberRole(member=member, add_roles=new_add, rem_roles=new_rem))

    def add_roles(self, member, role_list):
        # Tambahkan member dan rolenya sebagai MemberRole object sesuai urutan
...
class Settings(commands.Cog):
    ...
    async def prunelocalsettings(self, ctx):
        """Merubah pengaturan semua server menjadi default dan menghapus semua pengaturan yang telah di set dalam semua server (owner only)."""
